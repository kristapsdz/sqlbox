...
main(int argc, char *argv[])
{
	...
	struct sqlbox_pstmt	 pstmts[] = {
		{ .stmt = (char *)"CREATE TABLE foo (bar REAL)" },
	...
	if (res->ps[0].type != SQLBOX_PARM_FLOAT)
		errx(EXIT_FAILURE, "res->ps[0].type != SQLBOX_PARM_FLOAT");

	/* The original size should be ignored. */

	if (res->ps[0].sz != sizeof(double))
		errx(EXIT_FAILURE, "res->ps[0].sz != %zu", sizeof(double));
	if ((long long)rint(res->ps[0].fparm) != 20L)
		errx(EXIT_FAILURE, "res->ps[0].fparm !~ 20");
