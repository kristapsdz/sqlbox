...
main(int argc, char *argv[])
{
	...
	if (!sqlbox_role_hier_child(hier, 0, 2))
		errx(EXIT_FAILURE, "sqlbox_role_hier_child");
	if (!sqlbox_role_hier_start(hier, 3))
	...
	if (!(roles.roles[0].roles[0] == 1 &&
	    ...
	    !(roles.roles[0].roles[0] == 2 &&
	      roles.roles[0].roles[1] == 1))
	...
	if (roles.roles[3].rolesz != 3)
		errx(EXIT_FAILURE, "wrong role sizes");
	for (i = 0; i <= 2; i++) {
		for (j = 0; j < roles.roles[3].rolesz; j++)
			if (roles.roles[3].roles[j] == i)
				break;
		if (j == roles.roles[3].rolesz)
			errx(EXIT_FAILURE, "wrong roles");
	}
