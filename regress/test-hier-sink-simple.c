...
main(int argc, char *argv[])
{
	...
	if (!sqlbox_role_hier_sink(hier, 3))
		errx(EXIT_FAILURE, "sqlbox_role_hier_sink");

	if (!sqlbox_role_hier_gen(hier, &roles, 0))
		errx(EXIT_FAILURE, "sqlbox_role_hier_gen");

	if (roles.roles[0].rolesz != 3)
		errx(EXIT_FAILURE, "wrong role sizes");
	for (j = 1; j <= 3; j++) {
		for (i = 0; i < roles.roles[0].rolesz; i++) 
			if (roles.roles[0].roles[i] == j)
				break;
		if (i == roles.roles[0].rolesz)
			errx(EXIT_FAILURE, "wrong roles");
	}

	if (roles.roles[1].rolesz != 1)
		errx(EXIT_FAILURE, "wrong role sizes");
	if (roles.roles[1].roles[0] != 3)
		errx(EXIT_FAILURE, "wrong roles");

	if (roles.roles[2].rolesz != 1)
		errx(EXIT_FAILURE, "wrong role sizes");
