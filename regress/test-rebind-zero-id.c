...
main(int argc, char *argv[])
{
	...
	if (!sqlbox_rebind(p, 0, nitems(parms2), parms2))
	...
	if (res->ps[1].type != SQLBOX_PARM_INT)
		errx(EXIT_FAILURE, "res->ps[1].type != SQLBOX_PARM_INT");
	if (res->ps[1].iparm != 20)
		errx(EXIT_FAILURE, "res->ps[1].iparm != 20");
	...
	if (res->ps[0].type != SQLBOX_PARM_INT)
		errx(EXIT_FAILURE, "res->ps[0].type != SQLBOX_PARM_INT");
	if (res->ps[0].iparm != 30)
		errx(EXIT_FAILURE, "res->ps[0].iparm != 30");
	if (res->ps[1].type != SQLBOX_PARM_NULL)
		errx(EXIT_FAILURE, "res->ps[1].type != SQLBOX_PARM_NULL");
