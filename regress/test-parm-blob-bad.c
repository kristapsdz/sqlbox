...
main(int argc, char *argv[])
{
	...
	if (memcmp(res_blob_array, "abcd", 5))
	...
	if (memcmp(res_blob, "abcd", 5))
		errx(EXIT_FAILURE, "sqlbox_parm_blob_array value");
	free(res_blob);

	c = sqlbox_parm_float(&parm, &res_double);
	if (c != -1)
		errx(EXIT_FAILURE, "sqlbox_parm_float should fail");

	c = sqlbox_parm_int(&parm, &res_int);
	if (c != -1)
		errx(EXIT_FAILURE, "sqlbox_parm_int should fail");
	...
	if (c != -1)
		errx(EXIT_FAILURE, "sqlbox_parm_string_array should fail");
