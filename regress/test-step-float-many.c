...
main(int argc, char *argv[])
{
	...
	for (i = 0; i < parmsz; i++) {
		snprintf(tmpbuf, sizeof(tmpbuf), "col%.5zu REAL", i);
	...
	if (!(stmtid = sqlbox_prepare_bind(p, dbid, 1, parmsz, parms)))
		errx(EXIT_FAILURE, "sqlbox_prepare_bind");
	if ((res = sqlbox_step(p, stmtid)) == NULL)
		errx(EXIT_FAILURE, "sqlbox_step");
	if (res->psz != 0)
	...
	for (i = 0; i < parmsz; i++) {
		if (res->ps[i].type != SQLBOX_PARM_FLOAT)
			errx(EXIT_FAILURE, 
				"res->ps[%zu].type != "
				"SQLBOX_PARM_FLOAT", i);
		if ((size_t)rint(res->ps[i].fparm) != i)
			errx(EXIT_FAILURE, 
